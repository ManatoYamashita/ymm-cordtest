...
export const usePopulationData = (selectedPrefCodes: number[], prefectures: Prefecture[]) => {
  // 人口データを保持するための状態を定義。
    ...
    const fetchPopulationData = async () => {
      ...
      const newData = await Promise.all(
        selectedPrefCodes.map(async (code) => {
          const response = await resas('/api/v1/population/composition/perYear', { prefCode: code });
